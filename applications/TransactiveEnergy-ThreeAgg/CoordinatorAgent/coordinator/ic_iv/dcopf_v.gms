$ontext
Use given initial conditions for Current-Voltage ACOPF.
$offtext
...
$ifthen.ICverbose %verbose% == 1
$call gams "%filepath%dcopf.gms" --genPmin=%genPmin% --allon=%allon% --linelimits=%linelimits% --case="%case%" --obj=%obj% gdx=dctemp.gdx lo=3 --timeperiod=%timeperiod%
$else.ICverbose
$call gams "%filepath%dcopf.gms" --genPmin=%genPmin% --allon=%allon% --linelimits=%linelimits% --case="%case%" --obj=%obj% gdx=dctemp.gdx lo=3 --timeperiod=%timeperiod%
*o=/dev/null > /dev/null
$endif.ICverbose
...
$ifthen.ICcondensed %condensed% == 'no'
           + sum((j,c)$(branchstatus(j,i,c) or branchstatus(i,j,c)), V_LineIr.l(i,j,c))
