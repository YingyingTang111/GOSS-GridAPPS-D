...
# Get controller config folder files
controllerFolderName = "../ControllerAgent/config"
controllerNames = [os.path.basename(x) for x in glob.glob(controllerFolderName + '/*.cfg')]
...
controller_nodes = ["controller_1", "controller_2", "controller_3"]
...
for ind in range(len(aggregator_nodes)):
    ...
    controllers = {}
    controller= ''
    for ctl in controllerNames:
        controllerName = ctl[:ctl.find('_config.cfg')]
        if controller_nodes[ind] == controllerName:
            ...
            controllers[controller]['Q_min'] = {'propertyType': 'integer', 'propertyUnit': 'none', 'propertyValue': 0}
            controllers[controller]['Q_max'] = {'propertyType': 'integer', 'propertyUnit': 'none', 'propertyValue': 0}
