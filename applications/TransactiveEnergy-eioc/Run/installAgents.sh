...
for filename in $CONFIGS1; do
          python ../../../../volttronold/scripts/install-agent.py -s "$SOURCE1" -c "$filename" --tag "$TAG1"
done
...
for filename in $CONFIGS2; do
         python ../../../../volttronold/scripts/install-agent.py -s "$SOURCE2" -c "$filename" --tag "$TAG2"
done
...
for filename in $CONFIGS3; do
         python ../../../../volttronold/scripts/install-agent.py -s "$SOURCE3" -c "$filename" --tag "$TAG3"
done
